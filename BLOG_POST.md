...
我选择了 **Cloudflare D1**（SQLite 数据库）+ **Pages Functions**（Serverless API），原因很简单：

1. **免费额度够用** - D1 每天 5,000,000 次读取，个人项目绰绰有余
2. **全球部署** - Cloudflare 的边缘节点，速度快
3. **无需运维** - 不用自己搭服务器，数据库自动扩容
4. **环境变量** - 私密密码存在 Cloudflare Secrets，不进代码仓库

### 架构变化

**之前：**
```
前端 (Vue 3) → data.js (静态数据)
```

**现在：**
```
前端 (Vue 3) → Cloudflare Pages Functions (API) → D1 数据库
                    ↓
                  KV 存储（同步数据）
```
...
前端界面上，编辑模式下每个分类按钮都有编辑和删除图标，挺直观的。

### 3. 拖拽排序（这次修了好久）

**网站卡片拖拽**：用的 `vuedraggable`，成熟方案，问题不大。
...
const syncFromCloud = async () => {
  const response = await fetch(`/api/sync/read?userId=${syncAuthToken.value}`)
...
**清空数据库**（危险操作）
```bash
curl -X POST /api/debug/clear \
  -H "Authorization: Bearer PASSWORD"
```

**清理重复 URL**（保留最早记录）
```bash
curl -X POST /api/debug/cleanup
```
...
**原因**：`draggable="isCategoryEditModeActive"` 绑定的是字符串 `"false"`，不是布尔值 `false`。HTML5 的 `draggable` 属性只要有值就是可拖拽的，不管值是什么。

**解决**：改成 `:draggable="isCategoryEditModeActive"`，用 v-bind 绑定布尔值。

### 坑 2：内部按钮干扰拖拽

**现象**：拖拽时有时能触发，有时不能。

**原因**：分类按钮里嵌套的编辑/删除按钮拦截了鼠标事件。

**解决**：
```vue
<button @click.stop="editCategory(item.category)"
...
```bash
wrangler d1 create nav-website --production
```

### 步骤 2：执行 SQL 建表

```bash
wrangler d1 execute nav-website --file=schema.sql
```
...
不用自己搭服务器，不用管扩容，Cloudflare 全包了。开发效率提升不少。
...
**在线演示**：https://nav-website.pages.dev（示例地址）
