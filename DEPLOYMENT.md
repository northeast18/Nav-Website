# 🚀 导航网站部署指南 - 数据库迁移与云端部署

本文档说明如何将数据从 GitHub 代码中分离，迁移到 Cloudflare D1 数据库和 KV 存储，实现数据与代码完全分离。

---

## 📋 目录

1. [架构说明](#架构说明)
2. [Cloudflare 资源准备](#cloudflare-资源准备)
3. [本地开发测试](#本地开发测试)
4. [部署到 Cloudflare Pages](#部署到-cloudflare-pages)
5. [数据迁移](#数据迁移)
6. [浏览器扩展配置](#浏览器扩展配置)
7. [常见问题](#常见问题)

---

## 架构说明

### 数据存储架构

```
┌─────────────────────────────────────────┐
│           Cloudflare Pages                │
├─────────────────────────────────────────┤
│  前端 (Vue.js + Vite)                      │
│  - 网站展示                              │
│  - 用户交互                              │
│  - API 调用                              │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│     Cloudflare Pages Functions          │
├─────────────────────────────────────────┤
│  /api/websites/read   → 读取网站数据     │
│  /api/websites/add     → 添加网站         │
│  /api/sync/save       → 同步收藏数据     │
│  /api/private/verify  → 验证私密密码     │
│  /api/migrate         → 数据迁移         │
└─────────────────────────────────────────┘
                  ↓
┌──────────────┬──────────────────────────┐
│  D1 Database │     KV Storage          │
├──────────────┼──────────────────────────┤
│  网站数据     │  • 收藏夹                │
│  (结构化数据)  │  • 自定义排序              │
│               │  • 访问记录              │
│               │  • 点击统计                │
│               │  • 同步时间戳            │
└──────────────┴──────────────────────────┘
```

### 代码与数据分离

**之前（不安全）**：
- ❌ 网站数据硬编码在 `src/data.js`
- ❌ 密码硬编码在 `src/App.vue`
- ❌ 分享 GitHub 代码 = 泄露个人数据

**现在（安全）**：
- ✅ 网站数据存储在 Cloudflare D1
- ✅ 密码存储在 Cloudflare Secrets
- ✅ GitHub 只包含代码，不包含数据
- ✅ 浏览器扩展直接操作云端数据

---

## Cloudflare 资源准备

### 步骤 1：创建 D1 数据库

1. 访问 [Cloudflare Dashboard](https://dash.cloudflare.com/)
2. 进入 **Workers & Pages** → **D1**
3. 点击 **Create database**
4. 输入数据库名称：`nav-website`
5. 点击 **Create**

创建后你会看到：
```
Database ID: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

**记录这个 ID**，稍后需要配置到 `wrangler.toml`。

### 步骤 2：创建 KV 命名空间

1. 在 [Cloudflare Dashboard](https://dash.cloudflare.com/)
2. 进入 **Workers & Pages** → **KV**
3. 点击 **Create a namespace**
4. 命名为：`nav-website-kv`
5. 点击 **Add**

记录命名空间 ID（格式：`xxxxxxxxxxxxxxxx...`）。

### 步骤 3：设置私密密码

在终端运行：

```bash
# 安装 Wrangler CLI（如果还没安装）
npm install -g wrangler

# 登录 Cloudflare
wrangler login

# 设置私密密码环境变量
wrangler secret put PRIVATE_PASSWORD
```

输入你的密码（例如：`sbp844695`）。

---

## 本地开发测试

### 1. 启动开发服务器

```bash
npm run dev
```

服务器会运行在 `http://localhost:5173`

### 2. 验证功能

1. **网站加载**：检查是否能正常显示网站列表
2. **私密分类**：输入密码 `sbp844695`，应该能解锁
3. **浏览器扩展**：
   - 配置同步 ID
   - 右键收藏网站
   - 导入浏览器书签

### 3. 查看开发日志

```bash
# 查看服务器输出
tail -f /tmp/claude/-home-north-projects-Nav-Website/tasks/b2d9357.output
```

---

## 部署到 Cloudflare Pages

### 步骤 1：更新 wrangler.toml

编辑 `wrangler.toml`，替换 ID：

```toml
# D1 数据库绑定
[[d1_databases]]
binding = "DB"
database_name = "nav-website"
database_id = "your_actual_d1_database_id"  # 替换为实际 ID

# KV 命名空间绑定
[[kv_namespaces]]
binding = "NAV_KV"
id = "your_actual_kv_namespace_id"  # 替换为实际 ID
preview_id = "your_preview_kv_id"  # 替换为实际 ID
```

### 步骤 2：部署到 Cloudflare Pages

**方式 A：通过 GitHub 连接（推荐）**

1. 在 [Cloudflare Dashboard](https://dash.cloudflare.com/) 创建 Pages 项目
2. 连接你的 GitHub 仓库
3. 构建命令：`npm run build`
7. 点击 **Save and Deploy**

**方式 B：通过 Wrangler CLI**

```bash
# 构建
npm run build

# 部署
npx wrangler pages deploy dist --project-name=nav-website
```

### 步骤 3：配置环境变量

部署后，在 Cloudflare Dashboard：
1. 进入你的 Pages 项目
2. **Settings** → **Environment variables**
3. 添加变量：
   - **Variable name**: `PRIVATE_PASSWORD`
   - **Value**: 你的密码（如 `sbp844695`）
4. 保存并重新部署

---

## 数据迁移

### 首次部署后执行数据迁移

1. 确保项目已部署到 Cloudflare Pages
2. 在终端运行：

```bash
curl -X POST https://your-project.pages.dev/api/migrate \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer your_password"
```

或者使用浏览器开发者工具：
```javascript
fetch('https://your-project.pages.dev/api/migrate', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer your_password'
  }
}).then(r => r.json()).then(console.log)
```

迁移成功后会显示：
```json
{
  "success": true,
  "migrated": 123,
  "total": 123,
  "message": "迁移完成！新增 123 条，跳过 0 条，总计 123 条"
}
```

---

## 浏览器扩展配置

### 更新 API 地址

部署后，更新扩展设置：

1. 打开扩展设置
2. 将 API 地址从 `http://localhost:5173/api/sync/save` 改为 `https://your-project.pages.dev/api/sync/save`
3. 所有 API 端点会自动使用相同的域名

### 完整的 API 端点

部署后，以下 API 端点可用：

```
https://your-project.pages.dev/
├── api/websites/read      → 读取所有网站
├── api/websites/add       → 添加新网站
├── api/sync/save          → 保存收藏数据
├── api/sync/read          → 读取收藏数据
├── api/private/verify     → 验证私密密码
└── api/migrate            → 执行数据迁移
```

---

## 常见问题

### Q1: 开发模式看不到数据？

**A**: 开发模式返回模拟数据。你的实际数据在 `src/data.backup.js` 中。

**解决方案**：
- 开发时：数据来自 `src/data.backup.js`
- 生产环境：数据来自 D1 数据库

### Q2: 部署后私密分类打不开？

**A**: 检查环境变量是否配置：
1. Cloudflare Dashboard → Pages → Settings → Environment variables
2. 确认 `PRIVATE_PASSWORD` 已设置
3. 重新部署项目

### Q3: 浏览器扩展收藏失败？

**A**: 检查：
1. 同步 ID 是否配置
2. API 地址是否更新为生产地址
3. 浏览器控制台是否有 CORS 错误

### Q4: 如何管理网站数据？

**A** 两种方式：

**方式 1：通过 API（编程）**
```javascript
// 添加网站
fetch('https://your-project.pages.dev/api/websites/add', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer your_password'
  },
  body: JSON.stringify({
    name: '网站名称',
    url: 'https://example.com',
    category: '常用网站',
    desc: '网站描述'
  })
})
```

**方式 2：直接操作 D1 数据库**
1. Cloudflare Dashboard → D1 → nav-website
2. 点击 "Console"
3. 执行 SQL 查询

### Q5: 如何备份数据？

**A**: D1 和 KV 都会自动备份。如需手动备份：

```bash
# 导出 D1 数据
wrangler d1 execute nav-website --command="SELECT * FROM websites" --output=backup.json

# 导出 KV 数据
wrangler kv:usage get
```

---

## 文件结构

```
Nav-Website/
├── src/
│   ├── App.vue                    # 主应用（已改造）
│   ├── data.js                   # API 数据加载
│   └── data.backup.js           # 原始静态数据备份
├── functions/
│   └── api/
│       ├── websites/              # 网站数据 API
│       │   ├── read.js
│       │   └── add.js
│       ├── sync/                  # 同步 API（已有）
│       ├── private/               # 私密验证（新增）
│       │   └── verify.js
│       └── migrate.js             # 数据迁移（新增）
├── scripts/
│   └── migrate-to-d1.js          # 迁移脚本
├── schema.sql                     # D1 数据库结构
├── wrangler.toml                 # Cloudflare 配置
├── .dev.vars                     # 本地开发环境变量
└── extension/                    # 浏览器扩展
    ├── background.js
    ├── popup/
    ├── options/
    └── bookmarks/
```

---

## 下一步

1. ✅ 完成数据迁移架构改造
2. ✅ 实现数据与代码完全分离
3. ✅ 支持浏览器扩展直接操作云端数据
4. 🔄 可以分享 GitHub 代码而不泄露个人数据

你的导航网站现在完全云端化了！🎉
